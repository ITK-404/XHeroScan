...
public class HandleCheckpointManger : MonoBehaviour
{
    ...
    #endregion
    public bool flagOff = false; // nếu true: không đặt point / dừng chain

    public void StopPlacing()
    {
        flagOff = true;
        if (checkPointManager != null && checkPointManager.firstPoint != null)
        {
    ...
    void Start()
    {
        ...
        if (splitRoomManager == null)
            Debug.LogError("Chưa tìm thấy SplitRoomManager");
        if (checkPointManager == null)
            Debug.LogError("Chưa tìm thấy CheckpointManager");
    }
    ...
    private void DetermineSegmentForRoom(Room room, Vector2 aOrig, Vector2 bOrig,
        out Vector2 A, out Vector2 B, out bool insideInsideCase, out bool A_fromHit, out bool B_fromHit, out bool haveValidPair)
    {
    ...
    // ============ đồng bộ GO theo polygon, giữ reference ============
    void EnsureLoopGoSyncedInPlace(Room room, LoopMap map, CheckpointManager cm, GameObject prefab, float eps = 1e-3f)
    {
        if (map.CheckpointsGO == null)
        {
            map.CheckpointsGO = new List<GameObject>();
            if (!cm.AllCheckpoints.Contains(map.CheckpointsGO))
                cm.AllCheckpoints.Add(map.CheckpointsGO);
        }

        var pool = new List<GameObject>(map.CheckpointsGO);
        var ordered = new List<GameObject>(room.checkpoints.Count);

        for (int i = 0; i < room.checkpoints.Count; i++)
        {
            var v = room.checkpoints[i];
            Vector3 want = new Vector3(v.x, 0f, v.y);

            GameObject pick = null; int bestIdx = -1;
            float best = eps * eps;

            for (int j = 0; j < pool.Count; j++)
            {
                var go = pool[j]; if (!go) continue;
                Vector2 p = new Vector2(go.transform.position.x, go.transform.position.z);
                float d = (p - v).sqrMagnitude;
                if (d < best) { best = d; bestIdx = j; pick = go; }
            }

            if (pick == null)
            {
                pick = Instantiate(prefab, want, Quaternion.identity);
                pick.name = $"CP_{i:00}";
            }
            else
            {
                pick.transform.position = want;
        ...
        map.CheckpointsGO.Clear();
        map.CheckpointsGO.AddRange(ordered);
    }

    // ============ NEW: chèn 1 đỉnh vào đúng cạnh trong polygon + GO ============
    void InsertBoundaryVertex(Room room, LoopMap map, Vector2 main2D, GameObject mainGO, float edgeEps)
    {
        int n = room.checkpoints.Count;
        int insertIndex = -1;
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (PointOnSegment(main2D, v0, v1, edgeEps)) { insertIndex = i + 1; break; }
        }
        if (insertIndex < 0) return;

        int exactIdx = room.checkpoints.FindIndex(p => (p - main2D).sqrMagnitude <= edgeEps * edgeEps);
        if (exactIdx >= 0) insertIndex = exactIdx;
        else room.checkpoints.Insert(insertIndex, main2D);

        if (insertIndex <= map.CheckpointsGO.Count) map.CheckpointsGO.Insert(insertIndex, mainGO);
        else map.CheckpointsGO.Add(mainGO);
    }

    // ============ NEW: chỉ cắt wallLines tại một điểm (KHÔNG đụng polygon) ============
    void SplitWallLinesAtPoint(Room room, Vector2 point, float eps = 0.001f)
    {
        List<(int index, WallLine wall)> toSplit = new();
        ...
        for (int i = 0; i < room.wallLines.Count; i++)
        {
            ...
            if (PointOnSegment(point, w1, w2, eps))
            {
                toSplit.Add((i, wall));
            }
        }
        ...
        for (int j = toSplit.Count - 1; j >= 0; j--)
        {
            ...
            Vector3 p3 = new Vector3(point.x, 0f, point.y);

            WallLine firstHalf = new WallLine { start = wall.start, end = p3, type = wall.type };
            WallLine secondHalf = new WallLine { start = p3, end = wall.end, type = wall.type };
    ...
    // ============ UPDATED: spawn + insert-in-place + cut walls ============
    private GameObject SpawnEdgeCheckpoint(Room room, LoopMap map, Vector3 pos, Vector2 p2)
    {
        // 1) Đồng bộ GO với polygon hiện tại
        EnsureLoopGoSyncedInPlace(room, map, checkPointManager, checkPointManager.checkpointPrefab);

        // 2) Tạo GO cho đỉnh biên
        var go = Instantiate(checkPointManager.checkpointPrefab, pos, Quaternion.identity);
        go.transform.SetParent(null, true);

        // 3) Chèn đỉnh vào đúng cạnh trong room.checkpoints + map.CheckpointsGO
        InsertBoundaryVertex(room, map, p2, go, EDGE_EPS);

        // 4) Cắt các wallLines đi qua điểm này (KHÔNG chạm polygon nữa)
        SplitWallLinesAtPoint(room, p2, EDGE_EPS);
    ...
    {
        // --- Tắt hoàn toàn khi flagOff ---
        if (flagOff)
        {
            if (checkPointManager.firstPoint)
            {
                Destroy(checkPointManager.firstPoint);
                checkPointManager.firstPoint = null;
            }
        ...
        // --- Nếu chưa có firstPoint: tạo p1 rồi thoát (đợi p2) ---
        ...
        // --- Có firstPoint: xử lý đoạn firstPoint -> position ---
        ...
        foreach (Room room in RoomStorage.rooms.ToList())
        {
            ...
            if (A_onB && B_onB)
            {
                ...
                if (!room.checkpoints.Any(p => Vector2.Distance(p, A) < EDGE_EPS))
                    tempCreatedPoints.Add(SpawnEdgeCheckpoint(room, map, start, A));
                if (!room.checkpoints.Any(p => Vector2.Distance(p, B) < EDGE_EPS))
                ...
                if (ShouldSplitSegment(room, A, B, A_onB, B_onB))
                    roomsToSplit.Add(room);

                continue;
            }

            // ===== CASE 2: Một trên biên + một trong =====
            if ((A_onB && B_inside) || (B_onB && A_inside))
            {
                ...
                { mainGO = checkPointManager.firstPoint; }
                else
                {
                ...
                InsertBoundaryVertex(room, map, main2D, mainGO, EDGE_EPS);
                SplitWallLinesAtPoint(room, main2D, EDGE_EPS);
                ...
                AddManualIfMissing(room, newline);
                ...
                // NEW: xét split cho Biên–Trong
                if (ShouldSplitSegment(room, main2D, extra2D, A_onB, B_onB))
                    roomsToSplit.Add(room);

                continue;
            }

            // ===== CASE 3: Cả hai ở trong =====
        ...
        // ===== Sau vòng lặp: cập nhật/split/redraw =====
        if (anyRoomUpdated)
        {
        ...
        // ===== Chuỗi nối tiếp: giữ/move firstPoint tới điểm cuối =====
        if (checkPointManager.firstPoint == null)
        {
            checkPointManager.firstPoint = Instantiate(checkPointManager.checkpointPrefab, endWorld, Quaternion.identity);
            checkPointManager.firstPoint.transform.SetParent(null);
        }
        else
        {
...
private IEnumerator WaitAndSplitRooms(List<Room> rooms)
{
    ...
    // Sai số cho so khớp điểm↔điểm & điểm↔cạnh khi dọn
const float CLEAN_EPS = 0.002f; // 2 mm
const float CLEAN_EPS2 = CLEAN_EPS * CLEAN_EPS;

private void CleanupExtrasForRoom(Room room)
{
    if (room == null) return;

    // 1) Tìm các extra cần xoá (trùng đỉnh hoặc nằm trên biên)
    var toRemove = new List<Vector2>();
    foreach (var p in room.extraCheckpoints)
    {
        bool hitVertex = room.checkpoints.Any(v => (v - p).sqrMagnitude <= CLEAN_EPS2);
        bool hitEdge = !hitVertex && OnBoundary(room, p, CLEAN_EPS);
        if (hitVertex || hitEdge) toRemove.Add(p);
    }

    if (toRemove.Count == 0) return;

    // 2) Xoá các GameObject extra tương ứng + gỡ khỏi mọi list
    //    - checkPointManager.currentCheckpoints
    //    - MovePointManager.placedPointsByRoom[room.ID]
    var mpm = FindFirstObjectByType<MovePointManager>();

    // Xoá theo vị trí gần (2D)
    bool Near2D(Vector3 a, Vector2 b, float eps2)
    {
        float dx = a.x - b.x; float dz = a.z - b.y;
        return (dx * dx + dz * dz) <= eps2;
    }

    // currentCheckpoints
    for (int i = checkPointManager.currentCheckpoints.Count - 1; i >= 0; i--)
    {
        var go = checkPointManager.currentCheckpoints[i];
        if (go == null) { checkPointManager.currentCheckpoints.RemoveAt(i); continue; }
        if (go.CompareTag("CheckpointExtra"))
        {
            Vector3 pos = go.transform.position;
            if (toRemove.Any(p => Near2D(pos, p, CLEAN_EPS2)))
            {
                checkPointManager.currentCheckpoints.RemoveAt(i);
                Destroy(go);
            }
        }
    }
    ...
    // placedPointsByRoom
    if (mpm != null && mpm.placedPointsByRoom.TryGetValue(room.ID, out var list) && list != null)
    {
    ...
    // 3) Xoá các Vector2 trong room.extraCheckpoints
    room.extraCheckpoints.RemoveAll(p => toRemove.Any(rem => (rem - p).sqrMagnitude <= CLEAN_EPS2));

    // 4) (Tuỳ chọn) Khử trùng lặp còn sót
    var dedup = new List<Vector2>();
    foreach (var p in room.extraCheckpoints)
        if (!dedup.Any(q => (q - p).sqrMagnitude <= CLEAN_EPS2)) dedup.Add(p);
    room.extraCheckpoints = dedup;
}
